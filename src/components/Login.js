...
const handlePasswordToggle = (e) => {
  ...
  const loginx = (e) => {
    e.preventDefault();
    if (email === "" || pass === "") {
      alert("kullanıcı adı yada şifre boş bırakılmaz");
    } else {
      if (email === slice.email && pass === slice.password) {
        navigate("/menu", { replace: true });
        dispatch(login(false));
      } else {
        alert("şifre yada email hatalı");
      }
    }
  };
  ...
  return (
    <div className="d-flex justify-content-center align-items-center 100vh ">
      <div className="card px-5">
        <div className="card-img mb-3 mt-4 d-flex align-items-center justify-content-center">
          <img
  ...
  >
    {showPassword ? <FaEyeSlash /> : <FaEye />}
          ...
          <div className="d-grid gap-2 my-2">
            <button className="btn btn-primary " onClick={(e) => loginx(e)}>
              Login
            </button>
          </div>
          <div className="card-title text-center">Hesabın Yokmu ?</div>
          <div className="d-grid gap-2">
            <button
              className="btn btn-success mb-4"
              onClick={(e) => orientationxy(e)}>
              Kayıt Ol
            </button>
          </div>
        </form>
